  ...
  return (
    <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 w-64 border-r border-white/10 bg-brand-blue-900 text-white transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-20 shrink-0 items-center gap-3 border-b border-white/10 px-5">
        ...
        <div>
          <div className="font-bold text-white uppercase">Applicant Tracker</div>
          <div className="text-[11px] text-white/80 uppercase">HR Recruitment System</div>
        </div>
      </div>
      ...
      <div className="mt-auto px-4 pb-5 pt-10">
        ...
        <div className="mt-5 flex items-center gap-2 px-2 text-[12px] tracking-wide text-blue-200"><span className="h-1.5 w-1.5 rounded-full bg-brand-green-500" /> HR Recruitment System</div>
      </div>
    </aside>
  );
}
