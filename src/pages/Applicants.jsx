...
export default function Applicants() {
  ...
  const metrics = [
    ["Total applicants", applicants.length, "Records in your workspace", Users, "bg-brand-blue-50 text-brand-blue-600"],
    ["Positions applied for", new Set(applicants.map((item) => item.positionAppliedFor?.trim()).filter(Boolean)).size, "Distinct roles across applicants", BriefcaseBusiness, "bg-brand-blue-50 text-brand-blue-600"],
    ["Awaiting interview", applicants.filter((item) => ["For Initial Interview", "For Final Interview"].includes(item.status)).length, "Initial and final interview stages", CalendarDays, "bg-brand-yellow-100 text-brand-yellow-700"],
    ["Ready for onboarding", applicants.filter((item) => item.status === "Ready for Onboarding").length, "Applicants at the onboarding stage", CheckCheck, "bg-brand-green-50 text-brand-green-700"],
  ];
  ...
  return (
    <div className="workspace-page mx-auto max-w-[1600px] space-y-6 pb-6">
      ...
      {notice && <div role="status" className="flex items-center justify-between gap-3 rounded-xl border border-brand-green-200 bg-brand-green-50 px-4 py-3 text-sm text-brand-green-700">{notice}<button aria-label="Dismiss notification" onClick={() => setNotice("")} className={`rounded p-1 ${focus}`}><X size={16} /></button></div>}
      <section aria-label="Applicant overview" aria-busy={loading} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, detail, Icon, color]) => <div key={label} className={`${panel} metric-card p-5 sm:p-6`}><div className="flex items-center justify-between gap-2"><p className="text-sm font-medium text-slate-500">{label}</p><span className={`rounded-xl p-2.5 ${color}`}><Icon size={19} /></span></div><p className="mt-3 text-4xl font-semibold tracking-tight text-slate-950 tabular-nums">{loading || loadError ? "—" : value.toLocaleString()}</p><p className="mt-3 text-xs leading-5 text-slate-500">{detail}</p></div>)}</section>
      <section className={`${panel} overflow-hidden`} aria-busy={loading}>
        <div className="flex flex-wrap items-center justify-between gap-3 p-5 sm:px-6"><div className="flex items-center gap-3"><h2 className="font-semibold text-slate-900">Applicant directory</h2><span className="rounded-full bg-brand-blue-50 px-2.5 py-1 text-xs font-semibold text-brand-blue-600">{loading || loadError ? "—" : filtered.length}</span></div><p className="text-xs text-slate-500">All records · All time</p></div>
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">{!loading && !loadError && visible.map((item) => <tr key={item.id} className="transition hover:bg-slate-50/70">
            <td className="px-5 py-4"><div className="flex items-center gap-3"><span aria-hidden="true" className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-brand-blue-50 text-xs font-bold text-brand-blue-600">{(item.applicantName || "?").trim().split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><div><button disabled={busy} onClick={() => openModal(item)} className={`rounded text-left font-semibold text-slate-800 hover:text-brand-blue-600 ${focus}`}>{item.applicantName || "Unnamed applicant"}</button><p className="mt-0.5 text-xs text-slate-500">{item.email || "No email provided"}</p>{item.contactNumber && <p className="mt-1 text-[11px] text-slate-400">{item.contactNumber}</p>}</div></div></td>
