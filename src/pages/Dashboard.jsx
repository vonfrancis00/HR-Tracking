...
export default function Dashboard() {
  ...
  const metrics = [
    ["Total applicants", total, "All records in your workspace", Users, "bg-indigo-50 text-indigo-600"],
    ["Active pipeline", active, "Across application and hiring stages", Layers3, "bg-blue-50 text-blue-600"],
    ["Awaiting interview", (counts["For Initial Interview"] || 0) + (counts["For Final Interview"] || 0), "Initial and final interview stages", CalendarDays, "bg-amber-50 text-amber-600"],
    ["Ready for onboarding", counts["Ready for Onboarding"] || 0, "Applicants at the onboarding stage", CheckCheck, "bg-emerald-50 text-emerald-600"],
  ];
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6 pb-6">
      <header className="flex flex-col justify-between gap-5 pt-1 sm:flex-row sm:items-center">
        <div>
          <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-600">Recruitment workspace</p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button onClick={() => { setSaveError(""); setModalOpen(true); }} className={`inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700 ${focus}`}><UserPlus size={17} /> Add applicant</button>
        </div>
      </header>
      ...
      <section aria-label="Recruitment metrics" aria-busy={loading} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([label, value, detail, Icon, color]) => <div key={label} className={`${panel} p-5`}>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[1.65fr_1fr]">
        <section className={`${panel} overflow-hidden`} aria-busy={loading}>
          ...
          <div className="space-y-1 p-3 sm:px-4">{stages.map(([key, label], index) => <button key={key} disabled={loading || !!error} aria-pressed={status === key} onClick={() => setStatus(status === key ? "" : key)} className={`grid w-full grid-cols-[minmax(0,1fr)_60px_28px] items-center gap-3 rounded-lg px-3 py-2.5 text-left transition sm:grid-cols-[minmax(0,1fr)_120px_30px] ${status === key ? "bg-indigo-50 text-indigo-700" : "text-slate-600 hover:bg-slate-50"} ${focus}`}>
            ...
            <span className="h-1.5 overflow-hidden rounded-full bg-slate-100"><span className={`block h-full rounded-full ${index === stages.length - 1 ? "bg-emerald-500" : "bg-indigo-500"}`} style={{ width: `${percentage(counts[key] || 0)}%` }} /></span>
        ...
        <section className={`${panel} overflow-hidden`} aria-busy={loading}>
          ...
          <div className="p-6">
            ...
            {loading || error ? <p role="status" className="py-6 text-sm text-slate-500">{loading ? "Loading source data…" : "Source data is unavailable."}</p> : sources.length ? <div className="max-h-72 space-y-5 overflow-y-auto pr-1">{sources.map(([source, count]) => <div key={source}>
              ...
              <div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-indigo-500" style={{ width: `${percentage(count)}%` }} /></div>
      ...
      <section className={`${panel} overflow-hidden`} aria-busy={loading}>
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-slate-100 px-5 py-3 sm:px-6">
          ...
          {(search || status) && <button onClick={() => { setSearch(""); setStatus(""); }} className={`rounded px-2 py-1 text-xs text-indigo-600 ${focus}`}>Clear filters</button>}
        </div>
